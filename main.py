...
SELF_PING_URL = (os.getenv("SELF_PING_URL") or "").strip()
SELF_PING_INTERVAL = max(60, int(os.getenv("SELF_PING_INTERVAL", "840")))
SELF_PING_ENABLED = (os.getenv("SELF_PING_ENABLED") or "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
WELCOME_JOIN_TEXT = (
    "🪔 မင်္ဂလာပါ သီတင်းကျွတ်အချိန်အခါလေး သာယာလှပစေပါစေ ✨\n\n"
    "Bot အသုံးပြုနိုင်ရန် အောက်ပါ Channel များကို အရင် Join ပေးပါနော် 🏮"
)

WELCOME_SUCCESS_TEXT = (
    "✅ အောင်မြင်ပါသည်။ သီတင်းကျွတ်မုန့်ဖိုး Bot ကို စတင်အသုံးပြုနိုင်ပါပြီ 🪔\n\n"
    "🏮 သီတင်းကျွတ်မုန့်ဖိုး Bot မှ ကြိုဆိုပါတယ် ✨\n"
...
HELP_TEXT = (
    ...
    f"2️⃣ သူငယ်ချင်း ၁ ယောက်ကို ဖိတ်ခေါ်တိုင်း {REFERRAL_REWARD} ကျပ် ရရှိပါမည်။\n"
...
logger = logging.getLogger("thadingyut_referral_bot")
router = Router()
...
async def finalize_join_verification(user_id: int) -> tuple[bool, bool]:
    async with get_pool().acquire() as conn:
        async with conn.transaction():
            ...
            if joined_now:
                await conn.execute(
                    "UPDATE users SET is_join_verified = TRUE WHERE user_id = $1",
                    user_id,
                )
...
async def add_required_channel(
    channel_id: int, title: Optional[str], username: Optional[str]
) -> None:
    await get_pool().execute(
        """
...
def channel_title_text(channel: asyncpg.Record) -> str:
    return channel["title"] or (
        f"@{channel['username']}" if channel["username"] else f"Channel {channel['channel_id']}"
    )
        ...
        if url:
            ...
            rows.append(
                [
                    InlineKeyboardButton(
                        text=f"🏮 {title}",
                        callback_data=f"channel_nourl:{channel['channel_id']}",
                    )
                ]
            )
...
def build_referral_share_keyboard(ref_link: str) -> InlineKeyboardMarkup:
    share_text = quote_plus("🪔 သီတင်းကျွတ်မုန့်ဖိုး Bot ကို Join လုပ်ပြီး မုန့်ဖိုးရယူလိုက်ပါ ✨")
...
async def verify_user_memberships(
    bot: Bot, user_id: int, channels: list[asyncpg.Record]
...
async def show_join_required(
    target: Message | CallbackQuery, channels: Optional[list[asyncpg.Record]] = None
) -> None:
...
async def send_referral_info(message: Message, user_id: int) -> None:
    ...
    text = (
        ...
        f"အောင်မြင်စွာ စတင်အသုံးပြုပါက 1 ယောက်လျှင် {REFERRAL_REWARD} ကျပ် ရရှိမှာ ဖြစ်ပါတယ်\n\n"
...
async def send_public_payout_post(bot: Bot, name: str, method: str, number: str) -> None:
    post = (
        ...
        r"✨ *Real\-time payouts are ongoing\.\.\.*\n"
        r"🪄 လူများစွာ နေ့စဉ် ထုတ်ယူနေကြပါပြီ\!\n\n"
        r"🪔 *Invite friends & claim your Thadingyut money now\!*"
    )
...
async def send_private_queue_post(
    bot: Bot,
    tg_user: Any,
    name: str,
    method: str,
...
async def cancel_withdraw_flow(
    message: Message,
    state: FSMContext,
...
def build_http_response(
    ...
    headers = [
        ...
        f"Content-Length: {0 if head_only else len(body_bytes)}",
        "Connection: close",
        "",
        "",
    ]
    response = "\r\n".join(headers).encode("utf-8")
    return response if head_only else response + body_bytes


async def health_http_handler(
...
async def http_ping_url(url: str) -> tuple[bool, str]:
    def _request() -> tuple[bool, str]:
        request = Request(
            url,
            headers={
                "User-Agent": "ReferralBotKeepAlive/1.0",
        ...
        with urlopen(request, timeout=20) as response:
            ...
            return 200 <= int(status) < 400, str(status)
...
def resolve_self_ping_url() -> Optional[str]:
    if SELF_PING_URL:
        return SELF_PING_URL
    if APP_BASE_URL:
        return f"{APP_BASE_URL}/ping"
    return None


async def keep_service_awake() -> None:
    target_url = resolve_self_ping_url()
    if not SELF_PING_ENABLED:
        logger.info("Self-ping disabled")
        return
    if not target_url:
        logger.warning(
            "Self-ping enabled but no SELF_PING_URL or APP_BASE_URL provided; keepalive loop disabled"
        )
        return

    logger.info(
        "Self-ping loop enabled: %s every %s seconds",
        target_url,
        SELF_PING_INTERVAL,
    )

    while True:
        ok, detail = await http_ping_url(target_url)
        if ok:
            logger.info("Self-ping success: %s (%s)", target_url, detail)
        else:
            logger.warning("Self-ping failed: %s (%s)", target_url, detail)
...
async def start_handler(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await upsert_user(
            message.from_user,
            invited_by=parse_start_referrer(message.text),
        )
...
async def check_join_callback(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        if not allowed:
            await callback.answer(
                "❌ Channel အားလုံးကို Join လုပ်ပြီးမှ ဆက်လုပ်နိုင်ပါမည်။",
                show_alert=True,
            )
...
async def cancel_handler(message: Message, state: FSMContext) -> None:
    if await state.get_state():
        ...
        await message.answer(
            "ℹ️ လက်ရှိ ပယ်ဖျက်ရန် လုပ်ငန်းစဉ်မရှိပါ။",
            reply_markup=main_menu_keyboard(),
        )
...
async def withdraw_waiting_name(message: Message, state: FSMContext) -> None:
    ...
    if not valid_name_input(name):
        await message.answer(
            "❌ အမည်ကို မှန်ကန်စွာ ရိုက်ထည့်ပေးပါ။",
    ...
    async with lock:
        try:
            ...
            if not allowed:
                await state.clear()
                await callback.answer(
                    "❌ Channel Join အခြေအနေ မမှန်ကန်တော့ပါ။",
                    show_alert=True,
                )
                await show_join_required(callback, channels)
                return
            ...
            await state.clear()
            await callback.answer(
                "❌ အမှားတစ်ခု ဖြစ်သွားပါသည်။ နောက်မှ ထပ်စမ်းကြည့်ပါ။",
                show_alert=True,
            )
...
async def fallback_handler(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            "🏮 Menu မှ လုပ်ဆောင်ချက်တစ်ခုကို ရွေးပေးပါ ✨",
            reply_markup=main_menu_keyboard(),
        )
    except Exception as exc:
...
async def main() -> None:
    ...
    logger.info("Starting Thadingyut referral/withdraw bot")
    logger.info("Health endpoints ready at /, /health, /ping")
    ...
    keepalive_task = asyncio.create_task(keep_service_awake())

    try:
        done, pending = await asyncio.wait(
            {polling_task, health_task, keepalive_task},
        ...
        for task in (polling_task, health_task, keepalive_task):
            if not task.done():
                task.cancel()
